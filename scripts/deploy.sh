...
#
# Requires: mpremote (pip install mpremote) and a Pico running MicroPython
# with the ulab module. The Pico must be attached via USB-CDC serial.
#
...
mpremote \
    ...
    cp $SRC/apl_value.py :marple/apl_value.py + \
    ...
    cp $SRC/ibeam_registry.py :marple/ibeam_registry.py + \
...
# Optional WiFi credentials (pico_eval.py handles absence gracefully).
if [ -f scripts/WIFI_CONFIG.py ]; then
    mpremote cp scripts/WIFI_CONFIG.py :WIFI_CONFIG.py
fi
