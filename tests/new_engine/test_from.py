...
class TestFromVector:
    ...
    def test_vector_index(self) -> None:
        assert Interpreter(io=1).run("1 3 5⌷10 20 30 40 50") == APLArray([3], [10, 30, 50])

    def test_repeated_indices(self) -> None:
        assert Interpreter(io=1).run("3 3 1⌷10 20 30 40 50") == APLArray([3], [30, 30, 10])
...
class TestFromMatrix:
    def test_select_row(self) -> None:
        i = Interpreter(io=1)
        i.run("M←3 4⍴⍳12")
        assert i.run("2⌷M") == APLArray([4], [5, 6, 7, 8])

    def test_select_rows(self) -> None:
        i = Interpreter(io=1)
        i.run("M←3 4⍴⍳12")
        assert i.run("1 3⌷M") == APLArray([2, 4], [1, 2, 3, 4, 9, 10, 11, 12])
