...
class TestShape:
    def test_shape_vector(self) -> None:
        assert Interpreter(io=1).run("⍴1 2 3") == APLArray([1], [3])

    def test_shape_matrix(self) -> None:
        assert Interpreter(io=1).run("⍴2 3⍴⍳6") == APLArray([2], [2, 3])

    def test_shape_scalar(self) -> None:
        assert Interpreter(io=1).run("⍴5") == APLArray([0], [])


class TestReshape:
    def test_reshape(self) -> None:
        assert Interpreter(io=1).run("2 3⍴⍳6") == APLArray([2, 3], [1, 2, 3, 4, 5, 6])
...
class TestReverse:
    def test_reverse(self) -> None:
        assert Interpreter(io=1).run("⌽1 2 3") == APLArray([3], [3, 2, 1])
...
class TestEncodeDecode:
    def test_encode(self) -> None:
        assert Interpreter(io=1).run("2 2 2⊤7") == APLArray([3], [1, 1, 1])
...
class TestGrade:
    def test_grade_up(self) -> None:
        assert Interpreter(io=1).run("⍋3 1 4 1 5") == APLArray([5], [2, 4, 1, 3, 5])

    def test_grade_down(self) -> None:
        assert Interpreter(io=1).run("⍒3 1 4 1 5") == APLArray([5], [5, 3, 1, 2, 4])
...
class TestReplicate:
    def test_replicate(self) -> None:
        assert Interpreter(io=1).run("1 0 1/1 2 3") == APLArray([2], [1, 3])
...
class TestMembership:
    def test_membership(self) -> None:
        assert Interpreter(io=1).run("2 3∈1 2 3 4 5") == APLArray([2], [1, 1])
