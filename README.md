...
> Ongoing work includes the rank operator (`⍤`), other missing
> operators (`⍥`, `@`, `⌸`, `⌺`), Pico hardware compatibility, and
> test file reorganisation. See [`plan/post-scalar-cleanup.md`](plan/post-scalar-cleanup.md)
> for the open TODO list and the latest `plan/progress-*.md` file
> for recent activity. Expect the API surface to be stable but the
